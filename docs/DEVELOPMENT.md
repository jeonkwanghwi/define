...
- **무엇**: 메인 기록 화면에서 재정의 감지 시 노출되던 "생각의 변화 · 선택" 입력 카드 제거 — 쓰는 동안엔 신규/재정의 구분 신호 zero. 대신 `SaveConfirmation`에 리빌 단계 추가: 저장으로 2번째 이상이 된 단어는 완료 연출 끝에 "이 단어, N번째 정의예요 · 지난 생각과 비교해보기 →" 칩이 등장(탭 → 단어 타임라인). 리빌이 있으면 **자동 닫힘 없음** — 화면 탭으로만 닫힘(같은 날 후속 수정: 3.4초 자동 닫힘이 칩 읽는 중에 끊겨서 제거. 일반 저장은 1.8초 자동 닫힘 유지 + 탭으로 조기 닫기 가능). 변화 노트 사후 기록은 타임라인 길게 누르기 → "변화 노트 추가/수정" 기존 기능 그대로.

...
- **무엇**: `/auth` 진입 시 **방법 선택 step(`chooser`)** 신설 — 세로 3버튼: **이메일로 계속하기**(포인트색·→기존 폼) / **카카오 로그인**(#FEE500) / **Apple로 로그인**(#000, 다크 대비용 hairline border). 소셜 2개는 미구현 → 탭 시 시스템 알림 대신 **"준비 중 · 곧 만나요" 인라인 안내**. 로그인/회원가입 토글은 이메일 폼 안에만(소셜은 가입·로그인 일원). 뒤로: verify→form→chooser→홈.

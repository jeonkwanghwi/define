...
- **좌2 아바타 마을** — 2D 픽셀아트 목업 구동(`/village` dev 라우트, 백엔드 0). **2D/3D 렌더링 방향은 팀 논의 중**(§6 결정 브리프). 회고/검색/주간 회고는 대체 후보로 보류
...
│       │   │       ├── town.tsx              # 마을 (좌2, AuthGate placeholder)
...
| 마을 (`/town`) | 🟡 placeholder | 아바타 마을(광장 컨셉2) · 2D 픽셀아트 목업 별도(`/village`) · 2D/3D 논의 중 · 회원가입 필요 탭 |
...
- [ ] **탭2 아바타 마을 — 2D vs 3D 렌더링 결정 (팀 논의 중)** ★ — *2026-06-13 갱신*
  - **현황**: 2D 픽셀아트 목업 **구동 완료**(`/village` dev 라우트, 백엔드 0, 새 의존성 0). 탑다운 RPG 룩(잔디 격자 타일+지붕색별 집+나무·꽃+캐릭터), 빈 땅 탭→캐릭터 걷기, 집 탭→그 집 정의 시트. 에셋은 직접 생성(CC0, `scripts/gen-village-art.mjs`+`assets/village/*.png`). 작업 로그 2026-06-13 참조.
...
- **변경**: `(tabs)/mood.tsx`→`town.tsx`(route `/mood`→`/town`), 라벨 회고→마을, 아이콘 신규 `village`(라인 스타일 집) 추가. AuthGate/placeholder 문구를 마을 컨셉으로. `_layout.tsx` TAB_ORDER·주석 갱신. `mood` 아이콘은 셋에 보존(미사용, 데드코드 삭제 안 함).
- **주의**: 실제 마을 화면은 2D 목업(`/village` dev 라우트)에 별도 존재 — 탭은 아직 가입게이트 placeholder. 2D/3D 확정(§6) 후 탭에 본격 연결.
...
- **렌더러 교체 가능 구조(2D→3D 대비 핵심)**: 3층 분리 — ⓐ 월드 데이터(`data/village-mock.ts`, **0~1 비율 좌표**라 해상도·렌더러 독립) ⓑ 렌더러(`components/village/village-board.tsx`, 그리기+탭 보고만 하는 presentational, **여기만 3D로 교체**) ⓒ 화면 로직(`app/village.tsx`, 월드 상태+걷기). 비싼 부분(데이터·로직·시트)은 살고 얇은 렌더러만 버려짐.
...
- **접근**: 탭바에 없는 dev 라우트 `/village`. 웹 `http://localhost:8081/village` 또는 `router.push('/village')`. 기존 5탭·게이트 **무수정**(외과수술식, 통째로 삭제 쉬움).
...
- **검증**: `tsc --noEmit` 0 + `expo export --platform web` 0에러(`/village` 라우트 생성). **인터랙티브 실측**(playwright-core + 시스템 Chrome): 빈 땅 탭 → 아바타가 그 지점으로 **애니메이션하며 이동**(중간 프레임 보간 확인: 174→253→288), 집 탭 → 걸어간 뒤 정의 시트 오픈까지 캡처 검증.

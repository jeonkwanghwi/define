...
### 2026-07-09 — 회상 로딩 3D 연출: 책장 넘김 인디케이터 (임시 시안)
- **무엇**: "생각하는 중" 기본 스피너가 밋밋하다는 의견 → **3D 페이지 플립**(`ThinkingPages`)으로 교체. 은유 = 과거의 내가 옛 기록을 뒤적임: 포인트색 책등을 축으로 sepia 페이지가 perspective+rotateY로 계속 넘어감. WebGL(three.js) 없이 RN transform만 사용 — 네이티브·웹 공통, 의존성 추가 0. reduce-motion 시 정적 표시.
- **검증**: tsc + Playwright로 실제 채팅 전송 중 2프레임 캡처 — 플립 각도가 프레임마다 달라 애니메이션 동작 확인.
- **참고(임시)**: 시안 성격 — 톤/크기/속도는 팀 피드백 후 조정. 다른 로딩 지점이 생기면 재사용 가능.

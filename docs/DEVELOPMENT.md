...
- **좌2 마을** — 🔴 **방향 대전환(2026-06-28): 마을 자산 = 사용자 메모기반 AI 이미지 자동생성**(프리셋 파일 X). 기존 2D 픽셀아트 목업·CC0 에셋·2D/3D 렌더 논의를 **사실상 대체** → 자체 설계 사이클 필요(생성 파이프라인·비용·저장·아이템 모델 + 마이페이지 "아이템 보기"). 기존 `/village-demo` 목업은 참고용으로만 잔존. §6 갱신

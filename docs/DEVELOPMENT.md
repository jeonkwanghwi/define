...
- **이메일 인프라 (이메일 인증 + 비밀번호 재설정)** 🆕 — 현재 signup은 `@IsEmail` **형식만** 검사 → 아무 이메일이나 통과·소유 확인 0, **비밀번호 재설정도 없음**(오타 이메일=복구 불가). 둘 다 "이메일로 코드/링크 발송"이라 **한 번들**. 발송은 **AWS SES**(→ AWS 이관과 함께/직후, 지금 외부서비스 붙였다 SES로 재이전 방지). 방식 = **가입 시 강제 인증**(2026-07-24 결정 — 인증 완료 전 계정 비활성; 정크 계정 0 우선, 전환 마찰↑ 수용). 소셜 로그인은 검증된 이메일 제공 → 이메일/비번 경로는 소수화 가능. 크리티컬 패스(EAS/소셜/광고) 밖.

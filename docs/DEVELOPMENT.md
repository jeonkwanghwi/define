...
**🟡 다음 후보 (우선순위 미정)**
- **광장 2단계**(= 광장 탭 개발 회차, 탭 번호 아님): 추천(좋아요)+추천순 정렬 → **3단계** 신고/모더레이션
- **각 탭 실제 기능**: tab2 마을 / tab3 과거의 나(회상) (광장은 1단계 완료). 명시적 공개/비공개 동의는 배포·다유저 시
- **tab2 마을** — 🔴 **방향 대전환(2026-06-28): 마을 자산 = 사용자 메모기반 AI 이미지 자동생성**(프리셋 파일 X). 기존 2D 픽셀아트 목업·CC0 에셋·2D/3D 렌더 논의를 **사실상 대체** → 자체 설계 사이클 필요(생성 파이프라인·비용·저장·아이템 모델 + 마이페이지 "아이템 보기"). 기존 `/village-demo` 목업은 참고용으로만 잔존. §6 갱신
...
- **tab3 회상(과거의 나와 대화)** — **v1 구현 착수(2026-06-29)**, 설계 `docs/superpowers/specs/2026-06-29-recall-v1-design.md`(2026-06-21 대체). 내 엔트리+birthYear 페르소나, 나이/기간 필터, 인캐릭터 시간봉인, ephemeral 대화, **대화당 30잉크 차감**(재화 v1 연동), 동의=tab3 가벼운 1회(`recallConsentAt`). **열림 게이트=서로 다른 단어 20개**. **대화 모드=자유만+`mode` seam**(토론/질문생성/단톡 후속). OpenAI 키 준비됨.

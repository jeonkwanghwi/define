...
- **품질 배치** — Button `loading` 상태 / TextField `error` 상태 / 다크모드 대비 3곳 / `sm` 버튼 44px. (정체성보다 완성도)
- **엣지 화면 공통 헤더** — 회상 잠금·단어장 빈 상태·광장 로딩/에러·로그아웃 게이트엔 `AppHeader` 미적용(중앙정렬이라 상단 여백 처리 필요).
...
- **무엇**: 메인 기록 화면에서 재정의 감지 시 노출되던 "생각의 변화 · 선택" 입력 카드 제거 — 쓰는 동안엔 신규/재정의 구분 신호 zero. 대신 `SaveConfirmation`에 리빌 단계 추가: 저장으로 2번째 이상이 된 단어는 완료 연출 끝에 "이 단어, N번째 정의예요 · 지난 생각과 비교해보기 →" 칩이 등장(탭 → 단어 타임라인, 자동 dismiss 1.8s→3.4s로 연장). 변화 노트 사후 기록은 타임라인 길게 누르기 → "변화 노트 추가/수정" 기존 기능 그대로.
...
- **다음 할 일**: 스테이징 배포 시 `prisma migrate deploy`가 unique 인덱스 생성 — 배포 전 스테이징 DB 닉네임 중복 여부 확인(쓰기 경로가 없었으므로 시드 유저 외 전부 NULL 예상) + 백필 실행. 경합(P2002) 시 500 → 409 변환은 필요해지면.

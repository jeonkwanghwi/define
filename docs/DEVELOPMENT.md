...
- **이메일 인프라 (이메일 인증 + 비밀번호 재설정)** 🆕 — 현재 signup은 `@IsEmail` **형식만** 검사 → 아무 이메일이나 통과·소유 확인 0, **비밀번호 재설정도 없음**(오타 이메일=복구 불가). 둘 다 "이메일로 코드/링크 발송"이라 **한 번들**. 방식 = **가입 시 강제 인증**(2026-07-24 — 인증 완료 전 계정 비활성). *마찰 무충돌*: 익명우선이 first-touch 저마찰(무가입 사용)을 이미 보장 → 가입은 "계정 원함" 결심 순간이라 인증번호는 표준 패턴, 원칙과 안 부딪힘. 발송 = **AWS SES**(AWS 이관과 함께/직후, 지금 외부서비스 붙였다 SES 재이전 방지). **SES는 발송만(~10%)** — 코드/토큰 생성·저장·만료·엔드포인트(가입/인증확인/재전송)·`emailVerified` 계정 상태·인증 UI 화면·레이트리밋·비번 재설정 플로우는 우리 구현. (관리형 Cognito는 더 해주나 자체 auth 갈아엎어야 해 비추천.) 소셜 로그인은 검증 이메일 제공 → 이메일/비번 경로 소수화 가능. 크리티컬 패스(EAS/소셜/광고) 밖.
- **보상형 광고(AdMob) → 잉크 적립** 🆕 — 자발적(옵트인) 광고 시청 시 잉크 지급. **네이티브 필수 → EAS 선행**(소셜과 같은 배, AWS 무관). 로드맵상 **③번**(EAS→소셜→광고→AWS), 출시 전 포함. 선행: Google AdMob 계정. **부정행위 방지 = AdMob 서버사이드검증(SSV)로 서버가 실시청 확인 후 적립**(회상 봉인·IAP 검증과 동일 "클라 불신" 원칙). **보상 차등 아이디어(사용자 2026-08-09)**: "5초/15초/30초" 같은 광고 길이는 **우리가 못 정함**(광고주가 정함) — 대신 **광고 포맷별 보상 티어**로 의도 구현(짧은 리워디드 인터스티셜=적은 잉크, 긴 리워디드 비디오=많은 잉크). 티어·금액은 착수 때 설계. BM [PLANNING.md](./PLANNING.md) §6.
- **인앱결제(IAP) 연동** 🆕 — **실매출 공통 전제**(잉크 구매·PDF 내보내기·프리미엄 테마/폰트·구독). StoreKit(iOS)/Play Billing(Android) + **서버 영수증 검증**(구매 위조 방지 — 회상 봉인·광고 SSV와 동일 "클라 불신" 원칙). **본인인증 불요 검증됨**(2026-07-24): 스토어=merchant of record라 애플/구글이 결제·구매자 인증 처리, 개발사는 구현 안 함(계정 확인 D-U-N-S/개발자 확인은 별개 종이작업). 자체 PG(스토어 밖) 갈 때만 본인인증·전자금융 의무 발생. 네이티브 필수 → EAS 선행. 로드맵상 8/17 AWS 이후 유력. 기획·BM [PLANNING.md](./PLANNING.md) §6.
...
- **검증**: tsc 클린 + API E2E(기기A 2개 + 기기B 3개 업로드 → `GET /journal`이 합집합 5개 반환 → pull 시 스테일 기기가 최신 다 받음 확인). 브라우저 focus 이벤트 실동작은 스테이징 재배포 후 실기기 확인 권장.
- **알려진 한계(별도)**: `mergeEntries`가 로컬 우선(local wins)이라 **다른 기기에서 수정한** 정의는 id 충돌 시 전파 안 됨(새 단어=다른 id는 union으로 해결). 기기 간 "수정" 전파는 후속 과제.
- **다음**: 팀원이 겪은 건 **스테이징 배포본** → 이 수정 반영하려면 **Railway 재배포** 필요. 재배포 후 스테일 데탑에서 앱 열기/탭 복귀 시 최신 반영 확인.

### 2026-08-17 — 🔴 닉네임 "초기화" 버그 진단 + settings-store 고아 nickname 정리
- **증상(사용자)**: 닉네임 바꿨는데 며칠 뒤 들어오면 초기화된 느낌 + 디폴트 닉네임 미적용 느낌.
- **체계적 디버깅으로 배제**: ⒜ 백엔드 정상(curl E2E — signup 디폴트 '조용한 솔바람' 배정·updateNickname 재로그인 지속 확인) ⒝ dev-web.sh는 DB 안 밀어 계정·닉네임 유지 ⒞ JWT 90일+401 자동로그아웃 없음 ⒟ 프론트에 nickname 덮어쓰는 경로 없음(출석·동기화는 setBalance로 nickname 보존).
- **root cause(localStorage 증거로 확정)**: 닉네임 진실의 출처가 **settings-store(로컬/익명) → auth-store(계정/서버)** 로 이전됐는데, settings-store에서 nickname 필드를 **코드로만 제거하고 persist 키(`define-settings-v1`)를 안 올림** → localStorage에 옛 익명 닉네임이 **고아 데이터로 잔존**(`define-auth-v1.user.nickname="트럼프"` vs `define-settings-v1.nickname="팡팡ㅝㅜㅏ"` 불일치). 사용자가 느낀 "초기화"의 정체 = 익명 닉네임이 계정 기반으로 바뀌며 화면에서 사라진 **1회성 이전**(현재 진행형 버그 아님 — auth 닉네임은 정상 지속).
- **수정**: settings-store persist에 `version: 1` + `migrate`(v0의 잔여 `nickname` 필드 strip, themeMode 보존) 추가 → 다음 로드에 고아 데이터 제거. 지뢰(누가 나중에 settings에 nickname 재도입 시 옛 값 부활) 차단. tsc 클린.
- **디폴트 닉네임 결론**: 신규 가입엔 정상 작동. 기존 계정은 이미 사용자 지정 닉네임("트럼프")이 있어 디폴트가 안 보이는 게 정상.

### 2026-08-17 — 마이페이지 "곧 만나요"에서 PDF 내보내기 행 제거(단어장으로 이동했으므로)
- **무엇/왜**: PDF 내보내기 진입점이 단어장(tab4)으로 옮겨졌는데 마이페이지 "곧 만나요"에 그대로 남아 **중복·혼란** → 제거. (2026-08-09 PDF 항목에서 "티저는 유지" 판단했던 것 **번복** — 진입점이 한 곳이어야 깔끔.) `곧 만나요`엔 "프리미엄 테마·폰트"만 남음. tsc 클린.

### 2026-08-17 — 버그 제보/문의 진입점 = 마이페이지로 결정 + placeholder 선배치
- **결정(사용자)**: 버그제보/오류신고 버튼 위치 = **헤더 ❌ → 마이페이지 ✅**. 이유: 저빈도 액션이라 헤더(상시 노출·이미 4요소) 프리미엄 공간에 두면 혼잡·낭비 / 설정·지원의 표준 위치가 마이페이지 / 아바타로 어디서든 2탭. (헤더는 TestFlight 테스트 기간 즉시신고용으론 근거 있으나 TestFlight 자체 피드백이 대체, 정식 출시 후엔 거슬림.)
- **지금 한 것**: PDF 때처럼 **자리만 선배치** — `mypage.tsx`에 신규 "지원" 섹션(설정→잉크→**지원**→곧만나요→버전) + `버그 제보 · 문의`(icon send, 준비 중, disabled) 행. 기능(이메일 열기/인앱 폼→서버 저장/외부 폼 링크)은 추후 구현 시 결정.
- **검증**: tsc 클린.

### 2026-08-15 — TestFlight 배포 준비(Apple 계정 대기 중 할 수 있는 것) + iOS 아이콘 알파 제거
- **방향 재조정(사용자)**: "카카오 서버부터"가 아니라 **"현재 앱을 TestFlight에 올려 QA 시작"이 먼저**. 지금 앱은 이미 QA 가능(이메일 로그인·전 기능 동작), 소셜/광고는 다음 빌드에 얹음. 단 TestFlight 업로드는 **Apple $99 계정이 물리적 선행조건** → "TestFlight 먼저 = Apple 등록 먼저". 사용자가 Apple 계정 가입 진행 중, 그 전까지 할 수 있는 준비를 처리.
- **iOS 아이콘 알파 제거(실제 리젝 방지)**: `assets/images/define.png`가 **RGBA(알파 있음)** — iOS App Store 아이콘은 알파 있으면 업로드 리젝(ITMS-90717). 알파가 **전부 불투명(min/max 255)** 이라 RGB로 평탄화 → **겉모습 100% 동일**, 리젝 원인만 제거. android/스플래시/웹도 공유하지만 다 불투명이라 무영향. config 변경 0, 새 파일 0(최소 수정).
- **점검 결과**: `eas.json` 이미 `production` 프로필(store 배포) 있음 → 손댈 것 없음. 권한 문구(infoPlist) 필요 네이티브 모듈 **현재 없음**(카메라/위치/알림/추적 미사용). 빌드번호는 `autoIncrement`+`appVersionSource: remote`로 EAS 자동 관리.
- **산출물**: [TESTFLIGHT-RUNBOOK.md](./TESTFLIGHT-RUNBOOK.md) — Apple 계정 나오는 즉시 복붙 실행하는 체크리스트(STEP 0 가입 → 1 ASC 앱 생성 → 2 `eas build --profile production` → 3 `eas submit --latest` → 4 내부 테스터 배포). 내부 테스트(≤100·심사 없이 즉시)=팀 최적, 외부는 나중. 소셜/광고 얹을 때 추가될 infoPlist·ATT·개인정보 라벨도 메모.
- **다음(Apple 승인 후)**: 런북대로 첫 빌드→TestFlight→팀 QA. 그 뒤 병렬로 카카오 로그인 서버 선개발.

### 2026-08-09 — 출시 수순 확정: Apple $99 등록을 최우선 관문으로(→ TestFlight QA, 정식 공개는 마지막)
- **핵심 결정**: 다음 착수는 **Apple Developer $99 등록 + 첫 iOS 빌드**. 이 하나가 **소셜 로그인·AdMob 광고·팀 QA 세 개를 동시에 잠금해제**하는 첫 도미노(전부 네이티브 빌드 위에서만 실동작). 로드맵(EAS→소셜→광고→AWS) 자체는 불변, "그러니 $99를 **지금 당장** 걸자"로 확정.
- **용어 교정(중요)**: 목표는 **TestFlight(비공개 테스트)**지 **앱스토어 정식 공개가 아님**. 팀 QA는 **TestFlight 초대/공개 링크**로 달성(앱스토어 링크 아님). 정식 공개는 완성 후 마지막 단계 — 미완성 정식 제출은 애플 완성도 심사(가이드 2.1/4.2)로 리젝 + 이미지 손상이라 QA 도구로 못 씀. 즉 "미완성이 일반인에 노출"되는 시나리오는 정식 공개 버튼 전까진 물리적으로 불가.
- **가시성 트랙 정리**: TestFlight=내가 초대한 사람만(내부 100·외부 10,000, 내부는 심사 없이 즉시 갱신·외부는 베타심사 1회) / 정식=전 세계 검색 노출. 빌드는 아무 때나 몇 번이든 업로드 가능(TestFlight 반복 최적), 정식 업데이트만 매 버전 App Review(보통 24h 내).
- **비용**: Apple $99/년(자동 갱신, TestFlight·정식 공통 전제) / 참고 Google Play는 평생 1회 $25. 팀 구성 iOS 4:Android 1이라 iOS가 실질 필요 → $99가 실질 병목.
- **실행 순서(확정)**: ① 지금 Apple $99 가입 신청(승인 1~며칠 대기 관문이라 선접수) + Kakao·AdMob 무료 등록 → ② **승인 대기 중 병렬**로 카카오 로그인 **서버 로직 선개발**(앱 없이 curl 검증 가능) + 소셜 데이터모델(카카오id↔User·이메일 중복 병합·이메일 미제공) 설계 → ③ 승인 후 첫 EAS iOS 빌드 → TestFlight 팀 배포 → 애들 링크 QA → ④ 그 위에서 소셜/광고 앱 연결·실기기 검증 → ⑤ 완성 판단 시 정식 출시(수동/단계적 공개).
- **다음(재개 시)**: Apple 개인 등록 준비물 정리(D-U-N-S 불요 조건) + 실제 가입 착수, 그리고 카카오 서버 로직 선개발 시작.

### 2026-08-09 — PDF 내보내기 진입점 = 단어장(tab4)으로 이동 + placeholder 선배치
- **결정(사용자)**: "PDF로 내보내기"는 마이페이지가 아니라 **단어장(tab4) 리스트 상단**에 있는 게 맞음 — 계정/설정 공간이 아니라 "단어 타임라인 쭉 보다가 만들고 싶은" 그 맥락에서 눌러야 자연스러움. 단위는 **단어장 전체("내 사전 한 권")** (단어 하나 타임라인 아님).
- **성격 재확인**: PDF 내보내기는 **유료(IAP) 항목**(PLANNING.md §6 — 주력 매출=IAP에 포함). 실기능은 **IAP 인프라(EAS + 서버 영수증 검증) 이후**라 지금은 못 만듦. 크리티컬 패스(EAS/소셜/광고) 밖.
- **지금 한 것**: 배포 전 구현 예정이라 **까먹지 않게 자리만 선배치**. `journal/index.tsx` stat 3칸과 단어 리스트 사이에 **비활성 placeholder 버튼**("PDF로 내보내기 · 내 사전 한 권으로 소장하기 · 준비 중") 추가 — 점선 테두리+opacity 0.7로 미완성 신호. 구현 시 이 블록을 Pressable로 바꾸고 disabled 스타일만 제거하면 됨. 마이페이지 "곧 만나요" 티저는 그대로 유지(기대감 유발).
- **검증**: tsc 클린. (실기능·결제 게이팅 미구현 — 자리만.)

### 2026-08-09 — 출시 전 네이티브 외부 계정 정리 + EAS 빌드 병목 확인
- **EAS 현황**: 설정 100% 완료(eas-cli·로그인[robot 토큰]·프로젝트 연결·app 식별자 `com.define.app`·eas.json·expo-dev-client) — **남은 건 실제 빌드 1개**. 빌드 이력 0.
- **팀 기기 구성(사용자)**: iOS 4명(본인 포함)·Android 1명 → **Android만 빌드는 무의미**, iOS 빌드가 실질 필요.
- **iOS 실기기 설치 = Apple Developer($99/년) 사실상 필수**(검증): 무료 Apple ID(Personal Team)는 **내 폰에만·7일 만료·푸시 불가·타인 폰 설치 불가** → 팀 테스트 부적합. $99는 어차피 App Store 출시에 필수라 "지금 사나 나중 사나 같은 돈". → **Apple 가입이 iOS 빌드 전체의 병목.**
- **네이티브 기능별 필요 외부 계정(전부 — 이게 다임)**: ⒜ **Apple Developer $99/년** = iOS 빌드 + Apple 로그인(딸려옴) ⒝ **Kakao Developers**(무료) = 카카오 로그인 ⒞ **Google AdMob**(무료) = 보상형 광고. (Google 로그인은 계획에 없음 — Apple+Kakao만. 이메일 인증=AWS·IAP=스토어 계정은 별개 후속.) 셋 다 EAS 빌드 위에서만 실동작.
- **보상형 광고 항목 신설**(§0 다음 후보): 로드맵 ③번, SSV 서버 검증, "광고 길이 5/15/30초는 우리가 못 정함 → 포맷별 보상 티어로 구현" 메모.
- **다음(재개 시)**: Apple Developer 가입 신청(승인 1~며칠 → 먼저 걸어두기) + Kakao·AdMob 무료 등록. 승인 대기 중 **카카오 로그인 서버 로직**(앱 없이 curl 검증 가능) 선개발 → 승인되면 iOS 빌드 → 소셜/광고 앱 연결 → TestFlight 팀 배포. **소셜 데이터모델(카카오id↔User·이메일 중복 병합·이메일 미제공)** 설계 선행 필요.

### 2026-08-09 — 🔴 계정 간 단어장 오염 버그 수정 (로컬 소유자 태그)
- **버그(사용자 제보)**: 로그아웃해도 로컬 단어장이 남는데(`auth-store.logout`은 인증만 비움), 로그인 시 `runSync`가 **업로드(로컬 전체→새 계정) 먼저** 실행 → A 로그인·로그아웃 후 **B로 로그인하면 A의 단어가 B 계정 서버에 주입됨**(프라이버시 유출 + 데이터 오염, 광장에도 노출, 양방향 확산). **새 단어를 안 써도 발생**(잔여 다운로드 데이터만으로). clientId=`Date.now()+랜덤`이라 B 단어 통째 덮어쓰기(충돌)는 사실상 없음 — "주입/오염"이 정확한 성격.
- **근본 원인**: 로컬 단어장이 **계정별이 아니라 기기 전역** — "이 로컬 데이터가 누구 것"이라는 개념 부재.
- **수정(옵션② 소유자 태그)**: `journal-store`에 `ownerId`(주인 계정 id, null=익명) 추가. 로그인 시 `auth-store.reconcileForUser`가 판별 — ⒜ 주인≠로그인계정 → **계정 전환**: 업로드 금지 + 로컬 비우고(`resetLocalForAccount`) 서버 것만 다운로드 ⒝ 주인 null(익명)/일치 → 소유권 클레임 + 기존 양방향 sync(익명 마이그레이션 보존). 로그아웃은 journal 미변경이라 ownerId 유지(전환 판별 열쇠 — null로 바꾸면 버그 재발). `_layout`에 기존 로그인 유저 마이그레이션(하이드레이션 시 owner=null이면 현 계정 클레임). 경계 유지: auth-store는 journal-store 직접 import X, sync-journal 헬퍼(`getLocalOwner`/`claimLocalOwner`/`resetLocalForAccount`) 경유.
- **검증**: 실 UI+서버 E2E **19/19** — 버그 시나리오(A→로그아웃→쓰기→B 로그인): B 서버·로컬 오염 0, A 서버 오염 0(양방향), 로컬=B단어만·주인=B / 회귀: 같은계정 재로그인 시 로그아웃 중 쓴 단어 업로드 O, 익명 첫 로그인 마이그레이션 O. 커밋 fffe0e4. 실기기 최종 확인은 EAS 후 권장(웹 localStorage로 검증).
- **후속 메모(사용자 방향)**: 이런 계정/로컬 정합 이슈가 많으니, 로그아웃 상태일 때 **로그인 유도를 더 강하게**(공지·팝업) 하는 UX를 추후 강화 검토. (이번엔 버그만 수정.)
...
- 스크립트: `scratchpad/e2e-signup-full.mjs`(playwright, context per 계정 격리).

### 2026-07-24 — 로그인 방법 선택 화면 (이메일/카카오/Apple 3버튼)
- **무엇**: `/auth` 진입 시 **방법 선택 step(`chooser`)** 신설 — 세로 3버튼: **이메일로 계속하기**(포인트색·→기존 폼) / **카카오 로그인**(#FEE500) / **Apple로 로그인**(#000, 다크 대비용 hairline border). 소셜 2개는 미구현 → 탭 시 시스템 알림 대신 **"준비 중 · 곧 만나요" 인라인 안내**. **회원가입은 로그인 방법 고르기 전 결정 → 방법 선택 화면 하단에 "아직 회원이 아니신가요? 회원가입" 링크**(→이메일 가입 폼). 이메일 버튼은 "이메일로 로그인"(3버튼 다 로그인 통일·간격 s4), 회원가입은 하단이 주 진입 + **로그인 폼에 보조 "처음이신가요? 회원가입" 링크**(가입 폼엔 없음). 세그먼트 배너 `AuthModeToggle` 제거. 소셜은 가입·로그인 일원. 뒤로: verify→form→chooser→홈. 헤딩 "나만의 단어를 정의해보세요" 상단 고정, 버튼 3개만 세로 중앙.
- **소셜 로드맵 자리**: 실제 Apple/Kakao 연동(로드맵 §0 다음 후보)이 붙을 seam. 브랜드 로고는 실연동 시 공식 에셋으로(현재 텍스트+브랜드색; 아이콘 셋에 글리프 없음).
- **검증**: front tsc 클린 + 웹 E2E **10/10**(3버튼 노출·소셜 "준비중" 안내·이메일→폼 토글/입력칸 등장·폼 뒤로→chooser 복귀·pageerror 0). 스크린샷 육안 확인.
...
- **구현**: 도메인 칸이 **셀렉트 박스 자체가 드롭다운 트리거**(▾ 박스 내장, 별도 버튼 없음) — 탭하면 `EmailDomainDropdown`(박스 바로 아래로 FadeIn "스르륵", `shadows.md` 오버레이, relative+zIndex)이 열림. **"직접 입력"은 드롭다운 항목** → 고르면 박스가 편집칸으로 전환(막다른 상태 없음, 박스 안 ▾로 목록 재오픈; 목록 도메인 재선택 시 편집 모드 해제). `auth.tsx` 조합(`effectiveEmail = 아이디@도메인`, 로그인은 email 단일). **백엔드 무변경**(조합 이메일이 기존 `@IsEmail` 통과 — 직접입력 허용이라 서버 allowlist 불필요). ※ 반복: 바텀시트 → 박스 아래 앵커드 드롭다운(외부 ▾) → **박스 자체가 트리거**(사용자 요청, 최종).
...
### 2026-07-24 — 출시 전 로드맵 확정: EAS 네이티브 → 소셜 로그인 → 보상형 광고 → AWS(~8/17)
- **무엇**: 출시 크리티컬 패스 4개 순서 확정 — ① **EAS 네이티브 dev build**(소셜·광고·실본인인증이 전부 네이티브 모듈이라 웹 검증 불가 → 최우선 언락) ② **소셜 로그인**(Apple+Kakao) ③ **보상형 광고→잉크** ④ **~8/17 AWS 이관** 집중.
- **결정**: 본인인증 = 실연동 미루고 **PASS 목업 유지**(원화 결제 드라이버 없음 — 잉크는 출석·광고로만 수급, YAGNI). 실연동은 결제 도입 시.
- **의존성/함정**: ⒜ 소셜/광고/실본인인증 = 네이티브 필수 → **EAS 선행** ⒝ Apple 로그인은 Kakao 넣으면 App Store 4.8로 **강제(세트)** ⒞ 보상형 광고는 클라 위조("광고 봤어요") 방지 위해 **AdMob 서버사이드검증(SSV)로 서버 적립** — 회상 봉인과 동일 "클라 불신" 원칙 ⒟ 소셜/광고 신규 서버 코드는 **env 기반**으로 짜 AWS 컷오버가 "설정만"이 되게 ⒠ **AWS Postgres 이관**: Prisma 마이그레이션 provider 종속 → SQLite 히스토리 재생 불가·**Postgres 재베이스라인 + 스테이징 유저(16+) 데이터 이관** / **JWT_SECRET 반드시 주입**(dev 폴백 `dev-secret-change-me` 방치 시 auth+회상 대화토큰 위조 가능) / 하드코딩 localhost·Railway URL env화.
- **아키텍처 이점**: "인증≠프로필 분리" seam(2026-06-20)·`EXPO_PUBLIC_API_URL` seam이 이미 있어 소셜·URL 이관 마찰 낮음.
- **다음(병렬)**: (사용자) Apple Developer·Kakao·AdMob 계정/약관 착수 — 리드타임 있는 종이작업이라 지금 시작. (개발) **EAS dev build부터**. 기획·BM·타임라인은 [PLANNING.md](./PLANNING.md) 동일 날짜.

### 2026-07-24 — 회상 과금 우회 봉인 (서명 대화 토큰)
- **무엇**: "새 대화 vs 이어하기" 판정을 클라 `isNewConversation` 플래그 → **서버 서명 JWT 대화 토큰**(고정 2h TTL, `purpose:'recall-conv'`+`sub`)으로 이전. 새 대화(유효 토큰 없음)만 동의·잔액 검사 후 30잉크 차감·토큰 발급, 이어하기는 유효 토큰 제시로만 무료(재서명 X = TTL 고정, 롤링이면 무한연장). `isNewConversation:false` 위조로 무한 무료 대화하던 우회(2026-07-20/21 발견) 봉인. **동의·잔액 검사가 플래그 안에 갇혀 `false`로 함께 우회되던 곁버그도 봉인**(이제 게이팅 조건 `isNew`가 위조 불가). `jwt.secret` 재사용 → DB 마이그레이션·신규 env 0.
- **flat 유지**: "대화당 30 = 무제한 턴" 그대로(턴 상한 미도입 — 감성 몰입 보호·YAGNI). 미래 "자연스러운 작별" 소프트캡 확장구만 스펙에 메모(토큰에 턴 카운터 얹으면 확장).
- **변경**: `recall.service`(토큰 발급/검증 헬퍼 + `chat()` 재구성)·`recall.module`(JwtModule 등록)·`recall-chat.dto`(필드 교체)·`recall.controller`(반환타입) / 프론트 `recall-api`·`recall-chat`(conversationTokenRef 저장·재전송, startedRef는 나가기확인·init가드용 유지).
- **검증**: curl 계약 RED(우회 재현)→GREEN 4/4(새대화 -30·토큰 / 이어하기 무차감·에코 / 위조 -30 / 동의철회 403) + front·back tsc 클린 + 서브에이전트 태스크 리뷰 2건 클린(cross-token 재사용 차단·실패시 ref 안전 확인). 코드 식별자 중립 유지.
- 설계: `docs/superpowers/specs/2026-07-24-recall-bypass-seal-design.md`.

### 2026-07-21 — 작성기 입력창: auto-grow 폐기 → 고정 높이 박스(200px)로 단순화
- **결정(사용자)**: 타이핑에 따라 박스가 커지다 내부 스크롤로 전환되는 2단계 동작(140→200 성장 후 스크롤)을 없애고, **처음부터 성장 종점 크기(200px)로 고정**. 넘치면 바로 입력창 내부 스크롤. 이유: 커지는 과정에서 화면이 출렁이는 것 자체를 제거(박스 크기 불변 → 안정적).
- **수정**: `index.tsx`에서 auto-grow 기계 전부 제거(`inputContentHeight` state·`onContentSizeChange`·리셋 useEffect·`Math.min` 높이 오버라이드) → `definitionInput.height = INPUT_HEIGHT(200)` 고정. multiline TextInput은 내용이 높이를 넘으면 내부 스크롤(캐럿=마지막 줄 따라감)이 기본.
- **검증(실타이핑)**: 빈 상태부터 200px 고정 / 3줄(상한 내) 박스 안 커지고 내부 스크롤 없음 / 10줄+ 200 고정·내부 스크롤로 마지막 줄 보임 / 저장 후 빈 200 박스 복귀. tsc·핵심 루프 1~13(저장·리빌·타임라인)·편집삭제 14 통과. (이전 2단계 상한안 대비: 빈 박스가 200으로 더 크지만 화면 출렁임 0.)
- **경위**: 2026-07-20 무상한 auto-grow → 270자+에서 버튼이 탭바 밑으로 사라짐(실 뷰포트 664, 히어로가 절반 점유) → 상한 200 도입 → 최종적으로 성장 자체를 없애고 200 고정. **교훈**: Playwright `fill()`은 onContentSizeChange/캐럿 스크롤 미발화 → 입력창 검증은 `pressSequentially`(실키)로.

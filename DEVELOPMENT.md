...
- 자잘한 폴리시: 통계 보강(streak)
- changeNote 후속: 과거 entry 소급 메모, WordRow 변화 노트 미리보기
  - ⚠️ **제약**: 소급 수정 액션은 `changeNote` 필드만 갱신하고 `savedAt`은 절대 손대지 않는다. 정렬은 항상 **생성일(savedAt)** 기준 — 옛 entry 노트를 고쳐도 맨 위로 튀면 안 됨(수정일 정렬 금지).
...
- [ ] **회원가입 인증 방식** — 소셜(Google/Apple/Kakao) / 일반 / 게스트 마이그레이션 + 로그아웃
